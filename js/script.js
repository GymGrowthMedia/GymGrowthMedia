...
const root=document.documentElement; function raf(){currentX+=(targetX-currentX)*.16; currentY+=(targetY-currentY)*.16; root.style.setProperty('--x',currentX+'px'); root.style.setProperty('--y',currentY+'px'); requestAnimationFrame(raf)} raf();
addEventListener('mousemove',e=>{targetX=e.clientX; targetY=e.clientY});
// hero canvas
(function(){ const c=document.getElementById('hero-canvas'); if(!c) return; const ctx=c.getContext('2d'); let w,h,dpr=window.devicePixelRatio||1; let blobs=[],mx=0,my=0,sc=0;
...
// burger
const burger=document.querySelector('.burger'), mobile=document.getElementById('mobile-menu');
if(burger && mobile){ burger.addEventListener('click',()=>{const o=burger.classList.toggle('active'); burger.setAttribute('aria-expanded',o?'true':'false'); mobile.hidden=!o; document.body.style.overflow=o?'hidden':''; });
...
// cart overlay
const cartBtn=document.getElementById('cartBtn'), cartPanel=document.getElementById('cartPanel'), closeCart=document.getElementById('closeCart'), backdrop=document.getElementById('cartBackdrop');
